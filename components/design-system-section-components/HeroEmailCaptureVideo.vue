<template>
  ...
  >
    <div class="container mx-auto px-4 md:px-6">
      ...
      <!-- Video Section -->
      <div v-if="showVideo && (videoUrl || videoThumbnail)" class="max-w-4xl mx-auto">
        <div
          class="relative rounded-2xl overflow-hidden"
          :class="{ 'shadow-2xl': showVideoShadow }"
        >
          ...
          <video
            ...
            :muted="muted"
            class="w-full h-auto"
            :style="videoAspectRatio ? `aspect-ratio: ${videoAspectRatio}` : 'aspect-ratio: 16/9'"
            @ended="handleVideoEnded"
          ></video>

          <!-- Video Thumbnail with Play Button (shown when not playing) -->
          <div
            v-else
          ...
          >
            <img
              :src="videoThumbnail || 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=1200&h=675&fit=crop'"
            ...
            <div class="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/30 transition-all">
              <div class="w-20 h-20 md:w-24 md:h-24 rounded-full bg-white/90 flex items-center justify-center group-hover:bg-white group-hover:scale-110 transition-all shadow-xl">
                <svg
                  class="w-10 h-10 md:w-12 md:h-12 text-gray-900 ml-1"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
...
const props = withDefaults(defineProps<Props>(), {
  ...
  // Video
  videoUrl: '',
  videoThumbnail: 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=1200&h=675&fit=crop',
  videoThumbnailAlt: 'Video Thumbnail',
  videoAspectRatio: '16/9',
  showVideoShadow: true,
  showControls: true,
  autoplay: false,
  loop: false,
  muted: false,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const apiPayload = {
      ...
      subject: 'New Lead from Hero Email Capture with Video',
